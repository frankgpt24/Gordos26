...
# --- FUNCIÓN DE CARGA "BLINDADA" (FECHAS Y COMAS) ---
def cargar_datos():
    df_raw = conn.read(ttl=0)
    if df_raw is not None and not df_raw.empty:
        df = df_raw.copy()
        
        # 1. Limpieza de Pesos (Crucial para que aparezcan en la gráfica)
        # Convertimos a string, cambiamos coma por punto y luego a número
        df['Peso'] = df['Peso'].astype(str).str.replace(',', '.')
        df['Peso'] = pd.to_numeric(df['Peso'], errors='coerce')
        
        # 2. Limpieza de Fechas
        # Probamos convertir ignorando errores primero para ver qué tenemos
        df['Fecha'] = pd.to_datetime(df['Fecha'], dayfirst=True, errors='coerce')
        
        # 3. Eliminar lo que no sea válido (si el peso o fecha fallan, no se puede graficar)
        df = df.dropna(subset=['Fecha', 'Peso'])
        
        # 4. Normalizar (quitar horas para la gráfica) y Ordenar
...
if not st.session_state['logueado']:
    ...
    col_u.markdown(f"Conectado como: **{st.session_state['usuario_actual']}**")
    if col_l.button("Cerrar Sesión", use_container_width=True):
        st.session_state['logueado'] = False
    ...
    if not df.empty:
        ...
        for u in df['Usuario'].unique():
            ud = df[df['Usuario'] == u]
            if not ud.empty:
                p_ini = ud.iloc[0]['Peso']
                p_act = ud.iloc[-1]['Peso']
                perdido = p_ini - p_act
        ...
        with st.expander("➕ Registrar nuevo peso"):
            ...
            with st.form("form_reg"):
                ...
                if st.form_submit_button("Guardar Peso", use_container_width=True):
                    # Formato AAAA-MM-DD 0:00 y peso con coma para tu Sheets
    ...
    with c_del:
        with st.expander("🗑️ Borrar último registro"):
            if not df.empty:
                mis_d = df[df['Usuario'] == st.session_state['usuario_actual']]
                if not mis_d.empty:
                    # El índice real en el Excel original
                    idx_borrar = mis_d.index[-1]
                    val_borrar = mis_d.iloc[-1]
                    st.warning(f"Se borrará: {val_borrar['Peso']}kg del {val_borrar['Fecha'].strftime('%d/%m/%Y')}")
